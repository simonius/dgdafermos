...
module sdgsolver
        ...
        function SpeedEstSplit(gr, rt, u, bf, t)
                ...
                double precision :: t
                ! aux
                double precision :: SpeedEstSplit
                double precision, dimension(size(u, 3)) :: ui, uo
                double precision, dimension(size(u, 1)) :: tse
                double precision, dimension(2) :: v, n, x
        ...
        subroutine DSDG(gr, rt, du, u, bf, t, dt)
                ...
                double precision, parameter :: eps1 = 1.0D-8                            ! The assumed precision, sqrt of machine epsilon
                ! auxillary variables
                double precision, dimension(size(gr%edges, 1)) :: sigma, sigmaLO        ! Total entropy dissipation pred. per edge
                double precision, dimension(size(u, 1)):: lam                           ! disp scaling factor
                double precision, dimension(size(u, 1), size(u, 2), size(u, 3)) :: Gu   ! Disp directions
                double precision, dimension(size(u, 1)) :: dE, dEcorr                   ! Entropy dissipation needed and entropy dissipation of the direction
                double precision, dimension(size(u, 1)) :: Eflow                        ! Entropy change due to entropy flow
                double precision, dimension(size(u, 3)) :: ui, uo, uiLO, uoLO           ! inner and outer value of conserved variables
                double precision, dimension(size(u, 1), size(u, 2), size(u, 3)):: uLO   ! low order projection of the conserved variables
                double precision, dimension(size(u, 1)) :: Eb, Ec                       ! Entropy of the basis solution and of the correction.
                double precision, dimension(size(u, 1), size(u, 2), size(u, 3)) :: Upsu ! Solution after application of the positive conservative filter. 
                double precision, dimension(2) :: n, x                                  ! normal and local position
                double precision :: ced, bsed, edgelam                                  ! The combined entropy dissipation of two cells
                double precision :: eps                                                 ! the scaled machine precision
                double precision, parameter :: delta = 1.0D-1                           ! the stepsize for the FD disp. calc.
                ...
                do i=1, Nt
                        ...
                        Eb(i) = Eb(i)*gr%Vol(i)*2.0D0
                        Ec(i) = Ec(i)*gr%Vol(i)*2.0D0
                        ...
                        lam(i) = max(0.0D0, dE(i) - Eflow(i))/(eps - dEcorr(i))
                        ! We update dE as the negative part of the entropy equality violation. 
                        dE(i) = min(0.0D0, dE(i) - Eflow(i))
                end do
                !$OMP END PARALLEL DO
                bsed = 0.0
                ...
                do i=1, Ne
                        ...
                        if (gr%edges(i, 2) > 0) then            ! An inner edge
                                ...
                                do k=1, rt%Nbdt
                                        ...
                                        sigma(i) = sigma(i) + rt%wsurf(k)*EulerDispOpHLL(ui, uo, n)*gr%bl(gr%edges(i, 1),edge)
                                        sigmaLO(i) = sigmaLO(i) + rt%wsurf(k)*EulerDispOpHLL(ui, uo, n)*gr%bl(gr%edges(i, 1),edge)
                                end do
                                ...
                                do k=1, rt%Nbdt
                                        ui = u(gr%edges(i, 1), rt%Bi(k, edge), :)
                                        uo = MirrorConsVar(ui, n)
                                        sigma(i) = sigma(i) + 0.5D0*rt%wsurf(k)*EulerDispOpHLL(ui, uo, n)*gr%bl(gr%edges(i, 1),edge)
                                        uiLO = uLO(gr%edges(i, 1), rt%Bi(k, edge), :)
                                        uoLO = MirrorConsVar(uiLO, n)
                                        sigmaLO(i) = sigmaLO(i)+0.5D0*rt%wsurf(k)*&
                                ...
                                do k=1, rt%Nbdt
                                        ...
                                        sigmaLO(i) = sigmaLO(i) + 0.5D0*rt%wsurf(k)*EulerDispOpHLL(uiLO, uoLO, n)&
                        ...
                        ! adjacent to it.
                        if (gr%edges(i, 2) > 0) then                    ! An inner edge
                                eps = eps1*min(gr%Vol(gr%edges(i, 1)), gr%Vol(gr%edges(i, 2)))
                                ced = dEcorr(gr%edges(i, 1))
                                bsed = 0.5D0*(dE(gr%edges(i, 1)) + dE(gr%edges(i, 2)))
                                EdgeLam = min(0.0D0, (sigma(i) - bsed))/(ced - eps)
                                ...
                                eps = eps1*gr%Vol(gr%edges(i, 1))
                                ced = dEcorr(gr%edges(i, 1))
                                bsed = 0.5D0*dE(gr%edges(i, 1))
                                EdgeLam = min(0.0D0, (sigma(i) - bsed))/(ced - eps)
                                lam(gr%edges(i, 1)) = lam(gr%edges(i,1)) + EdgeLam
        ...
        subroutine SDG(gr, rt, du, u, bf, t, dt)
                ...
                ! auxillary variables
                double precision, dimension(2) :: n, x                          ! Normal and position
                double precision, dimension(size(u, 3)) :: uo                   ! outer value of conserved variables
                double precision, dimension(rt%Nbdt, size(u, 3)) :: fi          ! Intercell flux
                 double precision, dimension(size(u, 2), size(u, 3)) :: f, g    ! the nodal values of the flux at collocation points
                double precision, dimension(size(u, 2), size(u, 3)) :: h        ! time derivative before multiplication with inverse mass matrix
        ...
        subroutine PosLimiter(gr, rt, u)
                ...
                ! Auxillary variables
                double precision, dimension(size(u, 3)) :: avu
                double precision :: lam, minp, minrho
                integer :: i, j, k
                double precision, parameter :: rhobound = 1.0E-12
                double precision, parameter :: pbound = 1.0E-12

                !$OMP PARALLEL DO PRIVATE(avu, minp, minrho, lam, j, k) SHARED(u)
                do i=1, size(u, 1)
                        do k=1,size(u, 3)
                                avu(k) = dot_product(rt%w, u(i, :, k))*2.0
                        end do
                        minrho = minval(u(i, :, 1))
                        if (minrho < rhobound) then
                                lam = min(1.0, (rhobound-minrho)/(avu(1)-minrho))
                                do j=1,size(u, 2)
        ...
        subroutine SSPRK33SDG(gr,rt, unew, u, bf, t, dt)
                ...
                double precision, allocatable, intent(out) :: unew(:, :, :)
                double precision, allocatable, intent(inout) :: u(:, :, :)
                external :: bf
                double precision :: t, dt
                double precision, allocatable :: du(:, :, :), u1(:, :, :), u2(:, :, :)
                integer :: n, k, l
                n = size(u, 1)
                k = size(u, 2)
                l = size(u, 3)
        ...
        subroutine solSDGSSPRK33(gr, rt, ures, u0, bf, tend, cfl)
                ...
                type(Grid), intent(in) :: gr
                type(RefTriangle), intent(in) :: rt
                double precision, allocatable, intent(out) :: ures(:, :, :)
                double precision, allocatable, intent(inout) :: u0(:, :, :)
                external :: bf
                double precision, intent(in) :: tend, cfl
                double precision :: dt, t = 0.0D0
